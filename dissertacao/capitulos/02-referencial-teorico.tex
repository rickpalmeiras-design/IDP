...
ela atrai e que reaparecem no Capítulo~\ref{sec:metodo}. A esse quadro
\citeonline{autor2025} acrescentam uma segunda dimensão: o que decide o efeito
não é apenas quantas tarefas a tecnologia assume, mas se a remoção delas eleva
ou reduz a especialização exigida pelas tarefas remanescentes, sendo que a mesma
tarefa pode ser especializada em uma ocupação e pouco especializada em outra. A
previsão é de sinais opostos entre preço e quantidade: quando a automação reduz
a exigência de especialização, os rendimentos caem e o acesso à ocupação se
amplia; quando a eleva, os rendimentos sobem e o conjunto de trabalhadores
qualificados encolhe. A consequência para este trabalho é direta e reaparece no
Capítulo~\ref{sec:resultados}: um índice único de exposição por ocupação não
distingue os dois casos, e um desenho construído apenas sobre margens de
quantidade pode não alcançar o ajuste, se ele ocorrer no rendimento.
...
observado. Em direção semelhante, \citeonline{massenkoff2026} acompanham o
desemprego americano por exposição ocupacional depois do lançamento do ChatGPT e
não encontram aumento sistemático entre os mais expostos; a única evidência
sugestiva aparece na entrada de jovens de 22 a 25 anos em ocupações expostas,
cerca de 14\% menor no período posterior ao lançamento, resultado no limite da
significância convencional e ausente acima dessa idade. Trata-se de relatório
técnico de empresa, sem revisão por pares, e a medida de exposição vem do uso
observado de uma única plataforma comercial, ressalvas que limitam o peso a
atribuir ao resultado; ainda assim, o trabalho é explícito quanto ao alcance do
desenho, ao reportar a ausência de efeito junto com a magnitude que conseguiria
distinguir de zero, da ordem de um ponto percentual. Tomadas em conjunto, tornam plausível que a margem de ajuste mais
