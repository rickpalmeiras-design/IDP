...
As taxas de transição medidas na amostra aumentaram no período; como esse
aumento aparece em todas as faixas, parte relevante dele é comum às ocupações,
sem que isso exclua a participação de mecanismos associados à exposição.

No nível de associação condicional, a resposta é que existe um gradiente estatisticamente detectável. Condicionalmente aos efeitos
fixos de ocupação de origem e de UF-trimestre, à teletrabalhabilidade interagida
com o período e ao conjunto de covariáveis, ocupações de origem com uma unidade
a mais de AIOE apresentam, do pré para o pós, aumento cerca de 2,2 pontos
percentuais maior na transição para ocupação menos exposta e variação 1,4 ponto
percentual menor na transição para ocupação mais exposta, sem que o desenho
separe esse gradiente do canal mecânico descrito na
Seção~\ref{sec:desfechos}. Nas margens de
vínculo o sinal é o inverso do esperado pela leitura alarmista: maior exposição
associa-se a menos saída do emprego e a menos informalização, e a informalização apresenta gradiente negativo em relação à exposição e
positivo em relação à teletrabalhabilidade, sem que o desenho identifique qual
força explica o aumento agregado.
...
coleta telefônica de 2020 e 2021 e volta ao patamar anterior em 2021T4, uma descontinuidade que atravessa o
período pré e cuja causa ainda não foi demonstrada. Enquanto ela não for
investigada, todo contraste entre pré e pós que use a série inteira pode
refletir, em parte, diferenças entre modos de coleta. É esse fato
...
coleta presencial e o de coleta telefônica coincide com a mudança no modo de
coleta, mas sua causa não está demonstrada: coleta, mudanças reais do período,
composição amostral, atrito e codificação ocupacional podem contribuir.
Enquanto ela não for investigada, as estimativas que usam o período de 2020 e
2021 devem ser lidas com cautela. O primeiro passo é diagnóstico: apresentar as
séries dos desfechos e da retenção por faixa de exposição e verificar se a
quebra varia com a exposição, considerando que cada par liga dois trimestres e
pode ter origem e destino em modos de coleta diferentes. A partir daí, três
encaminhamentos são possíveis, em ordem crescente de ambição: restringir a
amostra a pares cujas duas entrevistas tenham coleta comparável, o que custa
parte relevante dos trimestres pré; modelar o período de coleta interagido com
a ocupação, como sensibilidade que não separa automaticamente medida e
economia; ou tratar a mudança de modo como choque de medida, o que exige uma
fonte independente para estimar e descontar o erro.
