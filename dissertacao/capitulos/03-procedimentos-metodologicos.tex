...
\begin{quadro}[htbp]
  ...
  \begin{tabular}{@{}p{0.9cm}p{3.9cm}p{4.5cm}p{5.0cm}@{}}
    ...
    04 & \texttt{04\_painel\_pnadc.py} \newline \texttt{src/pnadc/pareamento.py}
       & Vincula os registros da mesma pessoa em trimestres consecutivos no
         ambiente remoto, aplica as regras de identidade da
         Seção~\ref{sec:par}, calcula os desfechos e transfere para o ambiente
         local apenas os resultados agregados em células.
...
A fonte é a PNAD Contínua trimestral do Instituto Brasileiro de Geografia e
Estatística (IBGE), acessada pelo repositório público Base dos Dados
\parencite{basedosdados_pnadc}. A janela disponível vai do primeiro trimestre de
2019 ao quarto trimestre de 2025, o que deixa vinte e sete trimestres elegíveis
como trimestre de origem, de 2019T1 a 2025T3. A estimação principal usa todos
eles; a descontinuidade de medida que atravessa 2020 e 2021, discutida no
Capítulo~\ref{sec:resultados}, recai sobre parte dessa janela e é tratada como
limite conhecido, e não por restrição do período.

A pesquisa é domiciliar e de painel rotativo: cada domicílio selecionado é
entrevistado em cinco visitas trimestrais consecutivas e depois substituído.
Essa estrutura é o que torna possível \emph{tentar} ligar as entrevistas de uma
mesma pessoa em dois trimestres seguidos, mas a ligação não é automática:
depende das regras de identidade descritas na Seção~\ref{sec:par} e de a pessoa
ter sido reencontrada e informada nos dois trimestres. Por isso o próprio
pareamento é tratado como desfecho, no teste de atrito da
Seção~\ref{sec:desfechos}.

A unidade de observação subjacente é a transição individual entre dois
trimestres consecutivos, identificada pelo par pessoa-trimestre de origem. Cada
registro corresponde à entrevista de uma pessoa no trimestre $t$ ligada à
entrevista dessa mesma pessoa no trimestre $t+1$, quando o pareamento é
possível. A exposição e as covariáveis são medidas na origem, ao passo que os
desfechos são construídos pela comparação entre origem e destino. Essa
organização é o que permite investigar os movimentos dos trabalhadores entre
situações no mercado de trabalho, em vez de comparar estoques.

Para a estimação, as transições individuais são agregadas em \textbf{células}.
Cada célula reúne os registros com o mesmo trimestre de origem, a mesma unidade
da federação, a mesma unidade primária de amostragem e valores idênticos das
variáveis explicativas e das características necessárias à construção de todos os
desfechos: ocupação de origem e de destino, condição de ocupação no destino,
formalidade na origem e no destino, indicador de pareamento e mudança de código
ocupacional nas duas resoluções. São preservados, em cada célula, o número de
pessoas, a soma dos pesos amostrais e a soma dos quadrados desses pesos. A
Seção~\ref{sec:roteiro} descreve os procedimentos que asseguram essas condições.

Sob essas condições, a agregação preserva os coeficientes de mínimos quadrados
ponderados: como o desfecho e o vetor de regressores são constantes dentro de
cada célula, a estimação sobre células, ponderada pela soma dos pesos
individuais, é equivalente à estimação sobre os registros individuais.
Formalmente, se $\mathcal{C}$ é uma célula e $y_i = y_{\mathcal{C}}$ e
$x_i = x_{\mathcal{C}}$ para todo $i \in \mathcal{C}$, então
...
A agregação também preserva as somas dos escores por unidade primária de
amostragem, pois a unidade primária entra na definição da célula e, portanto,
cada célula está inteiramente contida em uma dessas unidades. A reprodução exata
dos erros-padrão da estimação individual exige, além disso, as mesmas convenções
de ponderação e as mesmas correções de amostra finita, que dependem do número de
observações usado no cálculo: a rotina empregada aqui conta células, ao passo
que a estimação individual contaria pessoas. As duas contagens quase coincidem,
porque a agregação praticamente não compacta a base: são 4.014.438 células para
4.017.289 transições, e 99,9\% das células contêm uma única pessoa. A comparação
direta entre os dois conjuntos de erros-padrão não é feita aqui, porque a
estimação individual exigiria justamente os microdados que o desenho não
manipula.

Cabe registrar o alcance desse argumento. A equivalência é de cálculo: ela
justifica estimar sobre células em vez de indivíduos, e nada mais. As hipóteses
de que depende a leitura dos coeficientes, entre elas a comparabilidade entre
períodos e a janela temporal empregada, são questão separada, tratada na
...
\begin{enumerate}[label=\alph*)]
  ...
  \item mesmo dia e mesmo mês de nascimento, com dia entre 1 e 31 e mês entre 1
        e 12;
  \item ano de nascimento diferindo em no máximo um ano, o que acomoda erro de registro;
...
Essas regras produzem uma correspondência plausível entre registros, e não uma
identificação certa: admitem falsos pares e perdem pares verdadeiros. Além
disso, o painel acompanha os moradores reencontrados no mesmo domicílio, e não a
trajetória de quem muda de residência entre as visitas.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[corte, right=6mm of c.east, anchor=west, yshift=-4mm] (rc)
      {Identidade reconstruída: domicílio, ordem, visita seguinte, sexo, dia
       e mês de nascimento, ano e idade coerentes, chave não duplicada};
    ...
    \node[corte, right=6mm of g.east, anchor=west] (rf)
      {Ramo próprio, a partir de D: origem formal e destino classificável};

    \foreach \x/\y in {a/ra, b/rb, c/rc, d/rd, e/re, g/rf}
...
tratados como zero, já que isso introduziria atenuação mecânica
...
416 códigos COD com exposição, a média é 0,0053 e o desvio-padrão é $\sigma_A = 0{,}9445$. Os coeficientes do
Capítulo~\ref{sec:resultados} são reportados \emph{por unidade de AIOE}. Como
uma unidade equivale a $1/\sigma_A \approx 1{,}06$ desvio-padrão entre esses
códigos, o efeito associado a exatamente um desvio-padrão de exposição é cerca
de 5,5\% menor que o coeficiente reportado; ``por unidade de AIOE'' lê-se,
portanto, como ``por aproximadamente um desvio-padrão de exposição entre
ocupações''. Ponderando por trabalhador, a média cai para
...
\begin{quadro}[htbp]
  ...
  \begin{tabular}{@{}p{3.5cm}p{6.3cm}p{4.9cm}@{}}
    ...
    Formal para informal
      & $1$ se o destino está ocupado e é informal; $0$ se está ocupado e é
        formal ou se não está ocupado
      & pares com origem formal e destino classificável \\
    \addlinespace[3pt]
...
Por construção, no desfecho de transição de formal para informal, destinos não
ocupados entram com valor zero, e destinos ocupados com formalidade indefinida
ficam fora do domínio. O desfecho mede, assim, a probabilidade de passar a uma
ocupação informal entre origens formais com destino classificável, e não a
probabilidade de informalidade condicional a continuar ocupado.

O desfecho de pareamento cumpre função distinta das demais. Ele não é um resultado de interesse substantivo, e sim um \textbf{teste de
atrito}, de alcance limitado. Se a exposição da ocupação de origem estivesse
associada a quem desaparece do painel, os demais desfechos, definidos apenas
sobre pares completos, estariam sujeitos a seleção. O coeficiente de
$A_c \times \text{Pós}_t$ nesse modelo, porém, verifica apenas se o gradiente de
pareamento em relação à exposição mudou entre os períodos: diferenças
permanentes de retenção entre ocupações são absorvidas pelo efeito fixo
ocupacional e não são testadas. Não rejeitar a interação, portanto, é o
resultado desejável, mas não estabelece que a exposição seja irrelevante para
quem permanece no painel.

O par de margens direcionais (mobilidade descendente e ascendente) é
sempre lido em conjunto, e nunca isoladamente. As duas margens têm uma
assimetria mecânica de nível: origens mais expostas têm mais destinos possíveis
abaixo delas no gradiente, e origens menos expostas, mais destinos acima (apenas
a ocupação de maior AIOE não pode subir, e apenas a de menor não pode descer). O
efeito fixo de ocupação absorve essa diferença permanente de nível, mas não a
sua interação com uma mudança geral de mobilidade. Em um exemplo simplificado,
se a probabilidade de trocar de ocupação no período $t$ é $m_t$ e a fração dos
destinos menos expostos que a origem $c$ é $F_c$, a probabilidade de descer no
gradiente é $m_t F_c$, e um aumento comum de mobilidade eleva essa probabilidade
em $\Delta m \, F_c$, mais nas origens com mais destinos abaixo delas. Esse
componente varia com a exposição da origem sem nenhuma mudança específica
associada à inteligência artificial, e o desenho atual não o separa do
gradiente estimado: o contraste entre períodos é informativo, mas não isola a
direção do movimento de seu volume.
...
origem. O parâmetro de interesse é o \emph{gradiente} de resposta por unidade de exposição (AIOE), e não o efeito de um tratamento binário
...
por exclusão. A contaminação temporal não se limita a esse trimestre: como cada
par liga dois trimestres, os pares com origem em 2022T3 têm destino em 2022T4,
já parcialmente posterior ao lançamento, e são classificados como pré.
...
\begin{enumerate}[label=\alph*)]
  \item $\alpha_c$ absorve o nível permanente de cada ocupação. O coeficiente
        $\beta$ compara, portanto, as mudanças entre pré e pós de ocupações com
        exposições diferentes, e não os níveis dessas ocupações; como o AIOE é
        constante dentro da ocupação, $\beta$ não acompanha uma mesma ocupação
        cuja exposição aumente;
  \item $\lambda_{ut}$ absorve os componentes aditivos comuns a cada UF em cada
        trimestre, inclusive o nível do ciclo macroeconômico nacional. Um mesmo
        choque nacional com incidência diferente por ocupação, porém, não é
        absorvido e pode covariar com a exposição;
  \item $T_c \times \text{Pós}_t$ ajusta o coeficiente de exposição pela
        teletrabalhabilidade potencial da ocupação, sob a mesma forma funcional
        do tratamento, dada a correlação de $0{,}702$ documentada na
        Seção~\ref{sec:exposicao}; o termo não mede nem elimina toda a
        reorganização do trabalho remoto;
...
preocupação: ele detecta mudança no gradiente de pareamento associada à
exposição, e não diferenças permanentes de retenção nem seleção por
características não observadas. Os coeficientes descrevem, assim, as
transições dos indivíduos elegíveis e reencontrados sob as regras de
pareamento, e não automaticamente as de todos os ocupados brasileiros.
